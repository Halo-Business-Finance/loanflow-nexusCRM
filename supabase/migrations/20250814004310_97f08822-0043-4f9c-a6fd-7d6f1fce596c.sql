-- Fix Blockchain Verification Data Security (Updated)
...
-- 1. Drop existing policies if they exist and recreate
DROP POLICY IF EXISTS "Admins can view immutable audit trail" ON public.immutable_audit_trail;
DROP POLICY IF EXISTS "Admins can insert into immutable audit trail" ON public.immutable_audit_trail;

-- Enable RLS on immutable_audit_trail table (used in the view)
