-- Phase 1: Critical RLS Policy Hardening (Fixed v3)
...
-- 8. Fix remaining overly permissive policies

-- Fix encrypted_fields access
DROP POLICY IF EXISTS "System can manage encrypted fields" ON public.encrypted_fields;
CREATE POLICY "Only admins can view encrypted fields" 
ON public.encrypted_fields 
FOR SELECT 
USING (has_role('admin'::user_role));
...
-- Fix data_integrity_checks access  
DROP POLICY IF EXISTS "System can perform integrity checks" ON public.data_integrity_checks;
CREATE POLICY "Only admins can view integrity checks" 
...
-- Fix device_fingerprints access
DROP POLICY IF EXISTS "System can manage device fingerprints" ON public.device_fingerprints;
CREATE POLICY "Only admins can view all device fingerprints" 
ON public.device_fingerprints 
FOR SELECT 
USING (has_role('admin'::user_role) OR auth.uid() = user_id);
...
-- Fix api_request_analytics access
DROP POLICY IF EXISTS "System can manage API analytics" ON public.api_request_analytics;
CREATE POLICY "Only admins can view API analytics" 
ON public.api_request_analytics 
FOR SELECT 
USING (has_role('admin'::user_role));

CREATE POLICY "System can insert API analytics" 
ON public.api_request_analytics 
FOR INSERT 
WITH CHECK (true);

-- 9. Create enhanced security logging function
...
-- 10. Create function to validate user data access
