-- Drop existing function with conflicting parameters
DROP FUNCTION IF EXISTS public.validate_sensitive_table_access(text, text, text);
...
-- Function to validate access to sensitive blockchain data
CREATE OR REPLACE FUNCTION public.validate_blockchain_access(
  record_id_param text DEFAULT NULL,
  operation_param text DEFAULT 'SELECT'
)
RETURNS boolean
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- Only admin and super_admin can access blockchain data by default
  IF user_role IN ('admin', 'super_admin') THEN
    is_authorized := true;
  ELSIF record_id_param IS NOT NULL THEN
    -- Users can access blockchain records for their own data
    SELECT EXISTS (
      SELECT 1 FROM public.leads l 
      WHERE l.id::text = record_id_param AND l.user_id = user_id_val
      UNION
      SELECT 1 FROM public.clients c 
      WHERE c.id::text = record_id_param AND c.user_id = user_id_val
      UNION
      SELECT 1 FROM public.contact_entities ce 
      WHERE ce.id::text = record_id_param AND ce.user_id = user_id_val
    ) INTO is_authorized;
  END IF;
...
FOR SELECT
USING (public.validate_blockchain_access(record_id, 'SELECT'));

CREATE POLICY "Admins can manage blockchain records securely"
ON public.blockchain_records
FOR ALL
USING (public.validate_blockchain_access(record_id, 'ADMIN'))
WITH CHECK (public.validate_blockchain_access(record_id, 'ADMIN'));
...
DROP TRIGGER IF EXISTS monitor_blockchain_access ON public.blockchain_records;
CREATE TRIGGER monitor_blockchain_access
...
DROP TRIGGER IF EXISTS monitor_approval_access ON public.approval_requests;
CREATE TRIGGER monitor_approval_access
