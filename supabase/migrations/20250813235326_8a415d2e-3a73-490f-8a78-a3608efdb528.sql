-- Fix email account token security vulnerability

-- First, verify encryption functions exist and create the trigger
CREATE OR REPLACE FUNCTION public.encrypt_email_account_tokens()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO ''
AS $$
BEGIN
  -- Only encrypt if tokens are not already encrypted (prevent double encryption)
  IF NEW.access_token IS NOT NULL AND public.decrypt_token(NEW.access_token) IS NULL THEN
    NEW.access_token := public.encrypt_token(NEW.access_token);
  END IF;
  
  IF NEW.refresh_token IS NOT NULL AND public.decrypt_token(NEW.refresh_token) IS NULL THEN
...
WITH CHECK (
  ...
  -- Ensure tokens are provided (will be encrypted by trigger)
